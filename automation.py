...
class Booking(object):
    ...
    def main(self):
        ...
        try:
            ...
            self.browser.get("https://www.irctc.co.in/")
            # login_page = self.browser.find_element_by_id("loginFormId")
            # login_page = self.browser.find_element_by_xpath("//a[contains(@class, 'loginText') and contains(text(), 'LOGIN')]")
            login_page = self.browser.find_element(By.XPATH, "//a[contains(@class, 'loginText') and contains(text(), 'LOGIN')]")
            # login_page.find_element_by_id("8820571").send_keys(self.values["UserID"])
            # login_page.find_element(By.NAME, "8820571").send_keys(self.values["UserID"])
            login_page.find_element(By.CSS_SELECTOR, "input[formcontrolname='userid']").send_keys(self.values["UserID"])
            
            # login_page.find_element_by_name("8918335").send_keys(self.values["Password"])
            # login_page.find_element(By.ID, "8918335").send_keys(self.values["Password"])
            login_page.find_element(By.CSS_SELECTOR, "input[formcontrolname='password']").send_keys(self.values["Password"])
            time.sleep(10)
